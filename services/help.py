...
"""
        ...
        # 上传说明按用户身份区分：白名单用户可使用手动「素材 上传」；
        # 非白名单用户仅能通过私聊直接发送素材自动上传。
        if is_whitelisted:
...
"""
            ...
            help_text += """📤 上传素材 (投稿模式)
   - 请直接私聊机器人发送图片、视频或文件，系统会自动上传到你的个人投稿文件夹。
   - 自动上传时会按日期归档到 `个人投稿文件夹/年月日` 子目录（如 `2026_08_29`），同目录已存在则复用当天文件夹。
   - 投稿模式下只能访问自己的投稿文件夹，自动上传会过滤后台禁用后缀名的文件。
   - 首次上传会自动生成个人文件夹 `<投稿根路径>/<你的QQ号>`。
...
"""
            ...
            # 仅高级白名单用户显示「白名单管理」指令
            if self.is_advanced_whitelisted_user(user_id):
                help_text += """🛡️ `素材 白名单 增加 <QQ号>` - 将用户加入【普通白名单】。
🛡️ `素材 白名单 删除 <QQ号>` - 将用户移出【普通白名单】。
🛡️ `素材 白名单 查看` - 查看当前白名单状态。
   - 本指令仅限【高级白名单】用户使用；【普通白名单】通过本指令管理，【高级白名单】只能在管理后台 WebUI 设置。
   - 素材上传相关：私聊素材会直接自动上传，普通/高级白名单在投稿权限上相同。
"""
