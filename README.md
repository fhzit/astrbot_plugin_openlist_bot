...
| 平台 | 私聊 | 群聊 | 说明 |
| --- | --- | --- | --- |
| aiocqhttp / OneBot（NapCat、Lagrange 等） | ✅ | ✅ | 完整支持，含群文件区上传自动转存 |
| QQ 官方机器人（qq_official） | ✅ | ✅ | 附件直链来自消息组件，无需 OneBot 接口 |

* **私聊**：收到图片、视频、文件即自动上传到 OpenList，需开启 `private_auto_upload`（默认开启）。
* **群聊（OneBot）**：群文件区上传会产生 `group_upload` 通知，开启 `group_file_auto_upload` 后自动转存到 `group_file_upload_path`；同一文件 30 分钟内只转存一次。
* **群聊（QQ 官方机器人）**：QQ 官方没有群文件区通知，群聊里发送的**文件**类型附件会按 `group_file_auto_upload` 开关自动转存；图片/视频不自动转存，避免刷屏，可用 `素材 上传` 手动处理。
* QQ 官方机器人的用户/群标识是 OpenID（非数字），插件内部按字符串处理，不影响目录与权限逻辑。
* 群角色的管理员校验依赖 OneBot 接口；QQ 官方机器人下跨群操作会因无法校验角色而被拒绝（本群内操作不受影响）。
